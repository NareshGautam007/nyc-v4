...
$('a.add-wishlist').click(function(){
 ...
 //alert('marked wish-list');
 var reload =false;
 var wish_list_id =$(this).data('id');
 var goto_site = $(this).data('goto');

 wishListCart('add',wish_list_id,reload);
});
...
function wishListCart(action,wish_list_id,reload){

var action_error_msg = lang.error_to_cart;
    if (action == 'add') {
        $('.add-wishlist a[data-id="' + wish_list_id + '"] span').hide();
        $('.add-wishlist a[data-id="' + wish_list_id + '"] img').show();
        var action_success_msg = lang.added_to_cart;
    }
    if (action == 'remove') {
        var action_success_msg = lang.remove_from_cart;
    ...
    $.ajax({
        type: "POST",
        url: variable.Wishlist,
        data: {wish_list_id: wish_list_id, action: action}
    }).done(function (data) {
        $(".dropdown-cart").empty();
        $(".dropdown-cart").append(data);
        var sum_items = parseInt($('.sumOfItems').text());
        ...
        if (action == 'add') {
            $('.add-wishlist a[data-id="' + wish_list_id + '"] span').show();
            $('.add-wishlist a[data-id="' + wish_list_id + '"] img').hide();
        }
    });
