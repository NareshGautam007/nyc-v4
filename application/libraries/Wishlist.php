<?php

defined('BASEPATH') OR exit('No direct script access allowed');

class Wishlist
{

    protected $CI;
    public $sumValues;
    /*
     * 1 month expire time
     */
    private $cookieExpTime = 2678400;

    public function __construct()
    {
        $this->CI = & get_instance();
        $this->CI->load->model('admin/Home_admin_model');
    }

    public function wishListCart()
    {
        if ($_POST['action'] == 'add') {
            if (!isset($_SESSION['wish_list'])) {
                $_SESSION['wish_list'] = array();
            }
            @$_SESSION['wish_list'][] = (int) $_POST['wish_list_id'];
        }
        if ($_POST['action'] == 'remove') {
            if (($key = array_search($_POST['wish_list_id'], $_SESSION['wish_list'])) !== false) {
                unset($_SESSION['wish_list'][$key]);
            }
        }
        @set_cookie('wish_list', serialize($_SESSION['wish_list']), $this->cookieExpTime);
        $result = 0;
        if (!empty($_SESSION['wish_list'])) {
            $result = $this->getCartItems();
        }
        // get items from db and add him to cart products list from ajax
        $loop = $this->CI->loop;
        $loop::getCartItems($result);
    }




    public function removeFromCart()
    {
        $count = count(array_keys($_SESSION['wish_list'], $_GET['delete-product']));
        $i = 1;
        do {
            if (($key = array_search($_GET['delete-product'], $_SESSION['wish_list'])) !== false) {
                unset($_SESSION['wish_list'][$key]);
            }
            $i++;
        } while ($i <= $count);
        @set_cookie('wish_list', serialize($_SESSION['wish_list']), $this->cookieExpTime);
    }

    public function getCartItems()
    {
        if ((!isset($_SESSION['wish_list']) || empty($_SESSION['wish_list'])) && get_cookie('wish_list') != NULL) {
            $_SESSION['wish_list'] = unserialize(get_cookie('wish_list'));
        } elseif (!isset($_SESSION['wish_list']) || !is_array($_SESSION['wish_list'])) {
            return 0;
        }
        $result['array'] = $this->CI->Public_model->getShopItems(array_unique($_SESSION['wish_list']));
        if (empty($result['array'])) {
            unset($_SESSION['wish_list']);
            @delete_cookie('wish_list');
            return 0;
        }
        $count_articles = array_count_values($_SESSION['wish_list']);
        $this->sumValues = array_sum($count_articles);
        $finalSum = 0;

        foreach ($result['array'] as &$article) {
            $article['num_added'] = $count_articles[$article['id']];
            $article['price'] = $article['price'] == '' ? 0 : $article['price'];
            $article['sum_price'] = $article['price'] * $count_articles[$article['id']];
            $finalSum = $finalSum + $article['sum_price'];
            $article['sum_price'] = number_format($article['sum_price'], 2);
            $article['price'] = $article['price'] != '' ? number_format($article['price'], 2) : 0;
        }
        $result['finalSum'] = number_format($finalSum, 2);
        return $result;
    }

    public function clearShoppingCart()
    {
        unset($_SESSION['wish_list']);
        @delete_cookie('wish_list');
        if ($this->CI->input->is_ajax_request()) {
            echo 1;
        }
    }

}
