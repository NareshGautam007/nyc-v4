...
class Categories_model extends CI_Model
{
    ...
    public function getShopCategories($limit = null, $start = null)
    {
        ...
        // $query="Select * from shop_categories_translations";
        // $arr = array();
        // foreach ($query->result() as $shop_categorie) {
        //     $arr[$shop_categorie->for_id]['info'][] = array(
        //         'abbr' => $shop_categorie->abbr,
    ...
    public function deleteShopCategorie($id)
    {
     ...
     function singlefile($files){
        $file=$_FILES["image"]["name"];
        $singletmp=$_FILES["image"]["tmp_name"];
        $dir= 'attachments/' .$file;
        $config["max_size"]="2084";
        $config["allowed_type"]="jpg|png|jpeg";
        move_uploaded_file($singletmp, $dir);
        return $file;
    ...
    {   
       ...
       foreach ($post['translations'] as $abbr) {
            ...
            $arr['image'] =!empty($_FILES) ? $this->singlefile($_FILES) :NULL;;
